#!/bin/bash
# Setup script for Midwest + West merged regions
...
echo "=========================================="
echo "Midwest + West Merged Setup"
echo "=========================================="
echo ""
echo "This will merge US Midwest and West regions."
echo "Coverage: IL, OH, MI, IN, AZ, CO, ID, MT, NM, UT, WY, etc."
echo ""
...
WEST_FILE="${FILES_DIR}/us-west-latest.osm.pbf"
MERGED_FILE="${FILES_DIR}/us-midwest-west-merged.osm.pbf"

MIDWEST_URL="https://download.geofabrik.de/north-america/us-midwest-latest.osm.pbf"
WEST_URL="https://download.geofabrik.de/north-america/us-west-latest.osm.pbf"
...
if [ ! -f "$WEST_FILE" ]; then
    echo -e "${YELLOW}Downloading West...${NC}"
    wget -O "$WEST_FILE" "$WEST_URL"
    echo -e "${GREEN}✓ West downloaded${NC}"
else
    SIZE=$(du -h "$WEST_FILE" | cut -f1)
    echo -e "${GREEN}✓ West: ${SIZE}${NC}"
fi

# Step 2: Merge files
echo ""
echo -e "${GREEN}Step 2: Merging Midwest + West...${NC}"
...
if [ $? -eq 0 ]; then
    ...
    WEST_SIZE=$(du -h "$WEST_FILE" | cut -f1)
...
if [ -f "ors-docker/config/ors-config.yml" ]; then
    sed -i.bak 's|source_file:.*|source_file: /home/ors/files/us-midwest-west-merged.osm.pbf|' ors-docker/config/ors-config.yml
    sed -i.bak 's|graph_path: graphs|graph_path: graphs/midwest-west|' ors-docker/config/ors-config.yml
...
mkdir -p ors-docker/graphs/midwest-west
...
echo "  West: AZ, CO, ID, MT, NM, UT, WY"
echo ""
...
echo ""
echo -e "${GREEN}You'll have cross-region routing between Midwest and West!${NC}"
