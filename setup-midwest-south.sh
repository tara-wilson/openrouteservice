#!/bin/bash
# Setup script for Midwest + South merged regions
...
echo "=========================================="
echo "Midwest + South Merged Setup"
echo "=========================================="
echo ""
echo "This will merge US Midwest and South regions."
echo "Coverage: IL, OH, MI, IN, TX, FL, GA, NC, etc."
echo ""
...
SOUTH_FILE="${FILES_DIR}/us-south-latest.osm.pbf"
MERGED_FILE="${FILES_DIR}/us-midwest-south-merged.osm.pbf"

MIDWEST_URL="https://download.geofabrik.de/north-america/us-midwest-latest.osm.pbf"
SOUTH_URL="https://download.geofabrik.de/north-america/us-south-latest.osm.pbf"
...
if [ ! -f "$SOUTH_FILE" ]; then
    echo -e "${YELLOW}Downloading South...${NC}"
    wget -O "$SOUTH_FILE" "$SOUTH_URL"
    echo -e "${GREEN}✓ South downloaded${NC}"
else
    SIZE=$(du -h "$SOUTH_FILE" | cut -f1)
    echo -e "${GREEN}✓ South: ${SIZE}${NC}"
fi

# Step 2: Merge files
echo ""
echo -e "${GREEN}Step 2: Merging Midwest + South...${NC}"
...
if [ $? -eq 0 ]; then
    ...
    SOUTH_SIZE=$(du -h "$SOUTH_FILE" | cut -f1)
    echo -e "${GREEN}✓ Merge complete!${NC}"
    echo "  Midwest: ${MIDWEST_SIZE}"
    echo "  South: ${SOUTH_SIZE}"
...
if [ -f "ors-docker/config/ors-config.yml" ]; then
    sed -i.bak 's|source_file:.*|source_file: /home/ors/files/us-midwest-south-merged.osm.pbf|' ors-docker/config/ors-config.yml
    sed -i.bak 's|graph_path: graphs|graph_path: graphs/midwest-south|' ors-docker/config/ors-config.yml
...
mkdir -p ors-docker/graphs/midwest-south
...
echo "  South: TX, FL, GA, NC, SC, VA, TN, AL, MS, AR, LA, OK, KY, WV"
echo ""
...
echo ""
echo -e "${GREEN}You'll have cross-region routing between Midwest and South!${NC}"
